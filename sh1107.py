...
class SH1107(framebuf.FrameBuffer):
    ...
    def __init__(self, width, height, external_vcc, delay_ms=200, rotate=0):
        ...
        if self.rotate90:
            super().__init__(
                self.displaybuf, self.width, self.height, framebuf.MONO_VLSB
            )
        else:
            super().__init__(
                self.displaybuf, self.width, self.height, framebuf.MONO_HMSB
            )
        self.init_display()
    ...
    def init_display(self):
        ...
        self.write_command(
            (_MEM_ADDRESSING_MODE | (0x00 if self.rotate90 else 0x01)).to_bytes(
                1, "big"
            )
        )
        self.write_command(
            _SET_PAGE_ADDRESS.to_bytes(1, "big")
    ...
    def poweron(self):
        ...
        time.sleep_ms(
            self.delay_ms
        )  # SH1107 datasheet recommends a delay in power on sequence
            ...
            for start_row in range(0, p * 8, 8):
                if pages_to_update & current_page:
                    for row in range(start_row, start_row + 8):
                        buffer_2Bytes[0] = (
                            row & 0x0F
                        )  # low column (low col. cmd is 0x00)
